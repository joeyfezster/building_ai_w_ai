...
class MiniPongEnv(gym.Env[np.ndarray, int]):
    ...
    def __init__(
        ...
        config: MiniPongConfig | None = None,
    ) -> None:
        super().__init__()
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        ...
        if self.ball_y <= 0 or self.ball_y >= self.cfg.height - self.cfg.ball_size:
            self.ball_vy *= -1
            self.ball_y = np.clip(self.ball_y, 0, self.cfg.height - self.cfg.ball_size)
        ...
        if self.ball_vx < 0 and self.ball_x <= self.cfg.paddle_width:
            if self.agent_y <= self.ball_y <= self.agent_y + self.cfg.paddle_height:
                self.ball_vx *= -1
                self.ball_x = self.cfg.paddle_width
                self.hits += 1
                self.rally_length += 1
        ...
        if (
            ...
            if self.opp_y <= self.ball_y <= self.opp_y + self.cfg.paddle_height:
                self.ball_vx *= -1
                self.ball_x = self.cfg.width - self.cfg.paddle_width - self.cfg.ball_size
            else:
                reward = 1.0
                self.agent_score += 1

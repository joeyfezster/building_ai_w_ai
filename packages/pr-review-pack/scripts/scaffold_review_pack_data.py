...
# Patterns for self-artifacts — files generated by the review pack itself.
# Excluded from "real work" stats so the reviewer sees actual code changes.
SELF_ARTIFACT_PATTERNS = [
    "docs/pr*_review_pack*",
    "docs/pr*_diff_data*",
    "docs/reviews/*",
    "docs/reviews/**",
    "zone-registry.yaml",
...
def build_header(
    ...
    total_additions = diff_data.get("total_additions", pr_meta.get("additions", 0))
    total_deletions = diff_data.get("total_deletions", pr_meta.get("deletions", 0))
    total_files = diff_data.get("total_files", pr_meta.get("changedFiles", 0))

    # Compute real-work stats by excluding self-artifacts (review pack outputs)
    all_files = diff_data.get("files", {})
    real_files = {
        f: d for f, d in all_files.items()
        if not any(fnmatch(f, p) for p in SELF_ARTIFACT_PATTERNS)
    }
    real_work_stats = None
    if len(real_files) < len(all_files):
        real_additions = sum(d.get("additions", 0) for d in real_files.values())
        real_deletions = sum(d.get("deletions", 0) for d in real_files.values())
        real_work_stats = {
            "additions": real_additions,
            "deletions": real_deletions,
            "filesChanged": len(real_files),
        }

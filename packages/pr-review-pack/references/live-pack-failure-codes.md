...
|---|---|---|---|
| `finding-location-mismatch` | An **anchor** location (a `locations[*]` with `context=False`, the default) has a `.file` that does not normalize to a path present in `diff_data.files`. The detail includes a Levenshtein closest-match suggestion. Locations with `context=True` are exempt — those are cross-references (where the fix should go, an analogous pattern, a related test) and are not validated against the diff. | YES | Resume the **owning reviewer agent** (Pattern A). The agent is identified by `details.examples[i].agent`. Prompt: re-emit a corrected `ConceptUpdate` with the right `locations[*].file`, OR mark the location `context: true` if it's a cross-reference, OR use the suggested path. |
| `finding-without-anchor` | A finding has one or more **anchor** locations (`context=False`) but none of them resolve to a file in the diff. The reviewer is describing something not in this PR — likely a hallucination, a stale path, or a finding that should have been marked entirely as context. Distinct from `finding-location-mismatch` (which fires per-anchor); this fires once per finding that has zero in-diff anchors total. | YES | Resume the **owning reviewer agent** (Pattern A). Prompt: either (a) the finding is a hallucination and should be retracted, (b) at least one location should have its path corrected to something in the diff, or (c) every location should be marked `context: true` if the finding is purely supplementary commentary. |
...
| `interaction-gate-empty` | Clicking a `.gate-review-card` did not reveal a `.gate-detail` body, or the body was empty. | YES | Re-run the assembler — `convergence.gates[*].detail` (HTML) is the source. If the source data has the field but the rendered element is still missing, the defect is in the renderer/template; address it at `scripts/render_review_pack.py` and `assets/template_v2.html`. |
| `architecture-assessment-invalid` | The rendered `architectureAssessment` object is missing a required field, has an invalid `overallHealth`, or its values disagree with the source `.jsonl` `_type: "architecture_assessment"` line. | YES | Resume the **architecture reviewer agent** (Pattern A). Append a corrected `architecture_assessment` line. If the mismatch is in fields the assembler computes, re-run the assembler. |
| `diff-data-missing` | The pack HTML does not contain a parseable `const DIFF_DATA_INLINE = {...};` block. The renderer inserts this block; its absence indicates a renderer or pipeline failure. Without it the spec cannot validate finding-location-mismatch or file-coverage-gap, so it must fail loud rather than silently passing. | YES | Re-run the renderer (`scripts/render_review_pack.py --diff-data ...`) ensuring `--diff-data` points at the assembler-produced diff data JSON. If the renderer is correctly invoked but still omits the block, the defect is in the renderer/template — fix at `scripts/render_review_pack.py` and `assets/template_v2.html`. |
| `pack-data-unparseable` | No `const DATA = {...};` occurrence in the pack HTML parses to the pack-data shape (top-level `header` and `agenticReview`). The spec walks every occurrence from the bottom up and validates shape because a pack reviewing this very repo will embed the spec's own source — which contains the literal `const DATA = ` — and lastIndexOf would otherwise land on the source listing. If even shape-validated parsing fails, the renderer emitted malformed JSON or skipped the marker. | YES | Re-run the renderer (`scripts/render_review_pack.py`) and verify the assembled `pr{N}_review_pack_data.json` is well-formed. If the renderer is correctly invoked but the rendered HTML still has no shape-valid block, the defect is in the renderer/template — fix at `scripts/render_review_pack.py` and `assets/template_v2.html`. |

...
| `pack-data-unparseable` | No `const DATA = {...};` occurrence in the pack HTML parses to the pack-data shape (top-level `header` and `agenticReview`). The spec walks every occurrence from the bottom up and validates shape because a pack reviewing this very repo will embed the spec's own source — which contains the literal `const DATA = ` — and lastIndexOf would otherwise land on the source listing. If even shape-validated parsing fails, the renderer emitted malformed JSON or skipped the marker. | YES | Re-run the renderer (`scripts/render_review_pack.py`) and verify the assembled `pr{N}_review_pack_data.json` is well-formed. If the renderer is correctly invoked but the rendered HTML still has no shape-valid block, this becomes `renderer-template-gap`. |

...
```bash
cd "${CLAUDE_SKILL_DIR}" && PACK_PATH="<absolute-path-to>/docs/pr{N}_review_pack_{base8}-{head8}.html" npx playwright test e2e/pack-validation.spec.ts
```

The `pack-validation.spec.ts` suite validates any review pack:
1. Validates all structural, functional, and visual elements (layout, sections, interactivity, zone filtering, theme toggle)
2. Removes the self-review banner from the HTML file as the final test (banner removal is the trust signal — its removal means "this pack was machine-validated")

A separate `review-pack-v2.spec.ts` file contains fixture-dependent tests for development/CI only — it requires fixture files generated by `generate_fixtures.py` and tests specific data states (READY/GAP/BLOCKED/NOFACTORY).

If tests fail, fix the issue in the review pack data and re-render (Phase 3), then re-run tests. Iterate until green.

**Do NOT create per-PR test files.** The baseline suite covers all structural and functional validation. Do NOT write any `.spec.ts` files outside of `${CLAUDE_SKILL_DIR}/e2e/`.
